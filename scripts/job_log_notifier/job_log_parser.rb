  ...
  module JobLogParser
    ...
    def self.parse_log_and_conf log, properties
      ...
      log.read.scan /(#{scan_until '.'})\./ do |raw_record,|
        ...
        cur = (record_hashes[type][record[idfield]] ||= {}).merge! record
        cur['_id'] = cur[idfield]
        cur['parent_id'] = [parent_prefix, cur[idfield][/#{parent_id_regex}/]].join if parent_prefix
        cur.delete idfield
        cur['type'] = type
      ...
      record_hashes.values.map{|hash| hash.values}.flatten << recordize_properties(properties)
