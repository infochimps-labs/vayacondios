...
describe Vayacondios::HttpServer do
  ...
  let(:err) { Proc.new{ |c| fail "HTTP Request Failed: #{c.inspect}" } }
    ...
    it 'can create arbitrarily' do
      ...
      with_api(Vayacondios::HttpServer) do |api|
        get_request({
          ...
          MultiJson.load(c.response).should == ["foo", 2]
        end
      end
    end
    it 'can update arbitrarily' do
      with_api(Vayacondios::HttpServer) do |api|
        put_request({
                      :path => '/v1/infochimps/itemset/foo/bar',
                      :body => MultiJson.dump(["bar", 1]),
                      :head => ({'X-Method' => 'PATCH', :content_type => 'application/json' }),
                    }, err)
      with_api(Vayacondios::HttpServer) do |api|
        get_request({
