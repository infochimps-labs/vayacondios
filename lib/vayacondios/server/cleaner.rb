...
class Vayacondios
  ...
  # @attr [Array] counts tracks the number of cleaned documents by collection
  class Cleaner
...
DESCRIPTION
        ...
        s.define :organization, description: "Name of the organization within which to track events for this cleaner",  default: 'vayacondios', required: true
    ...
    def clean
      ...
      matching_collections.each do |collection|
        self.counts << { collection: collection, count: clean_collection(collection) }
      end
    end

    # Finshes this cleaner's run by announcing counts of deleted
    # records to Vayacondios (unless in "dry-run" mode) as well as
    # printing them out.
    def finish
      announce_counts unless settings[:dry_run]
      counts.each do |count|
        puts [count[:count], count[:collection]].map(&:to_s).join("\t")
      end
    end
    ...
    # Announces an event in Vayacondios consisting of the
    # per-collection counts of records deleted by this cleaner.
    #
    # The organization for the announcement can be controlled by the
    # command-line `--organization` option.  The topic will be
    # `vayacondios_cleaner`.
    def announce_counts
      database.collection("#{settings[:organization]}.vayacondios_cleaner.events").insert(t: Time.now, d: counts)
    end
